...
def _compute_wind_cost(x, grids, solver, profile, debug=False, verbose=False):
    ...
    ##f = open("Jo.bin", "wb") # output Jo for checking
    ##Jo_grid.tofile(f)
    ##f.close()
    ...
    if solver.continuity is None:
        ...
        ##f = open("Jc.bin", "wb") # output Jc for checking
        ##Jc_grid.tofile(f)
        ##f.close()
	...
	# modified by oue: added Js_grid
        ...
        ##f = open("Js.bin", "wb") # output Js for checking
        ##Js_grid.tofile(f)
        ##f.close()
    ...
    if solver.background is None:
        ...
        ##f = open("Jb.bin", "wb") # output Jb for checking
        ##Jb_grid.tofile(f)
        ##f.close()
